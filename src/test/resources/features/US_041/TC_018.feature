Feature:US_041 As an administrator, I want the support ticket page to be in order to be able to handle the opened support tickets.
  ...
  Scenario:TC_018 On the Update Edit page that opens when clicking the edit link in the select tab under the Action heading,
  it should be confirmed that a new category is saved with the Add New link next to the Category List.
    ...
    When "Update Ticket" page, the user clicks on the "Add New" link next to the "Category List" title
    Then click on the "Add New" link, enter the Category name in the "Name" box and then click on the "save" button
    And tests that a new "Category List" has been added.